<template>
  <div class="mr-root roomDa willy_serviceChat">
    <van-nav-bar title="客服聊天" left-arrow @click-left="onClickLeft">
    </van-nav-bar>
      ...
      <ul style="margin-bottom: 90px;min-height: 400px">
        ...
        <li :class="{right_wechat:mess.sendernickname==userName,lift_wechat:mess.sendernickname!=userName}" v-for="mess in mySendMessage">
          <template v-if="mess.sendernickname&&mess.sendernickname!=userName">
            ...
            <p class="willy_Pone" style="text-align: center;margin: 0 auto;background-color: #dfdfdf;width: 50%" v-text="mess.mySendTime">
              2018-05-29 09:21</p>
...
export default {
  ...
  methods: {
    loadList() {
      ...
      vm.$axios.get(url, { params }).then(response => {
        ...
        if (response.data.statusCode == -100) {
          ...
          const _list = (response.data.resultInfo || []).map(a =>
            JSON.parse(a.msgContent)
          )
          _list.reverse()
    ...
    initWebSocket() {
      ...
      const vm = this
      vm.newUuid =
        'youke' + new Date().getTime() + Math.floor(Math.random() * 11) + 10

      if (localStorage.getItem('userInfo')) {
        vm.websock = new WebSocket(
          'ws://47.106.11.246:8086/websocket?chatType=1&token=' +
            vm.userData.token
        )
      } else {
        vm.websock = new WebSocket(
          'ws://47.106.11.246:8086/websocket?chatType=1&chartId=' +
            vm.newUuid +
            '&userId=' +
            vm.newUuid
        )
