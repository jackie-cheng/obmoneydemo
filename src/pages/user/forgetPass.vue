  ...
  <div class="mr-root">
    ...
    <div class="forgetPass">
      <van-cell-group v-show="!messSuccess">
        <van-field v-model="phoneNum" placeholder="请输入您的手机号" type="number" icon="clear" onkeyup="this.value=this.value.slice(0,11)" @click-icon="phoneNum = ''" />
        <van-field class="forgetTwoInput" center v-model="photoCode" placeholder="请输入图片验证码">
          <template slot="button"><img :src="photoCodeUrl" @click="showPhotoCode()" ref="Imgdata"></template>
          <!--<van-button slot="button" size="small" type="primary" v-if="!disableBut" @click="obMobileCode()">发送验证码</van-button>-->
        </van-field>
        <van-field center v-model="messageCode" placeholder="请输入短信验证码">
          <van-button slot="button" size="small" type="primary" v-if="!disableBut" @click="obMobileCode()">发送验证码</van-button>
      ...
      <van-cell-group v-show="messSuccess">
        <van-field v-model="newPass" placeholder="请输入您的新密码" icon="clear" @click-icon="newPass = ''" />
      </van-cell-group>
...
export default {
  ...
  methods: {
    ...
    showPhotoCode() {
      ...
      vm.$axios
        .get(`/api/Registercontroller/photoCode`)
        .then(response => {
    ...
    obMobileCode() {
      ...
      vm.$axios
        .post(`/api/Registercontroller/sendMsg`, param)
        .then(response => {
    ...
    goLogin() {
      ...
      vm.$axios
        .post(`/api/Registercontroller/judgeCerificationCode`, param)
        .then(response => {
          if (response.status == 200) {
    ...
    updateNewPwd() {
      ...
      vm.$axios
        .post(`/api/userLoginController/updateNewPwd`, param)
        .then(response => {
          if (response.status == 200) {
            if (response.data.status.indexOf('success') != -1) {
